...
from home_security.data.postgresql.models import (
    Household,
    Camera,
    ImageSet,
    IntrusionImage,
...
class StorageDriver:
    ...
    def add_image_set(
        ...
        household_id = (
            ...
            .filter(Household.name == household_name)
            .all()[0]
            .id
        )
        if not self.session.get(Camera, camera_id):
            logger.info(
                f"New camera feed detected at {household_name}, adding {camera_id} to DB!"
            )
            ...
            logger.info(
                f"Got new images for camera {camera_id} at household {household_name}"
            )

        imgset = ImageSet(
            received_date=datetime_obj, camera_id=camera_id, inferenced=False
        )
    ...
    def get_uninferenced_image_sets(self) -> List:
        uninferenced_set_ids = (
            self.session.query(ImageSet).filter(ImageSet.inferenced == False)
        ).all()
        return uninferenced_set_ids

    def get_images_from_set(self, image_set: ImageSet) -> List[Image.Image]:
        images = []
        for image in image_set.intrusion_images:
            images.append(
                self.minio.get_image(
                    image_uuid=str(image.id), bucket_name="intrusionimages"
                )
            )
        return images
